...
import com.example.admin.dto.minDto;
import io.minio.*;
...
public class UpdateService {
    ...
    public String UpdateImg(InputStream input, String username, String filename, String contenttype) throws ServerException, InsufficientDataException, ErrorResponseException, IOException, NoSuchAlgorithmException, InvalidKeyException, InvalidResponseException, XmlParserException, InternalException, InsufficientDataException, ErrorResponseException, InvalidResponseException, XmlParserException {
        ...
        minioclient.putObject(PutObjectArgs.builder().contentType(contenttype).
                bucket(bucket).object(filename).stream(input, input.available(), -1).build());
        String url="http://localhost:9000/image"+username+"/"+filename;
    ...
    public List<minDto> List(String username , String prefix) throws ServerException, InsufficientDataException, ErrorResponseException, IOException, NoSuchAlgorithmException, InvalidKeyException, InvalidResponseException, XmlParserException, InternalException {
        ...
        for (Result<Item> result : results) {
            ...
            System.out.print(result.get().etag());
            minDto dto=    new minDto(result.get().objectName(),result.get().isDir());
              name.add(dto);
        }
          return name;
    }
    ...
    public void create(String bucket) throws ServerException, InsufficientDataException, ErrorResponseException, IOException, NoSuchAlgorithmException, InvalidKeyException, InvalidResponseException, XmlParserException, InternalException {
        boolean exist=minioclient.bucketExists(BucketExistsArgs.builder().bucket(bucket).build());
        ...
        if(exist!=true){
            ...
            String txt="！！!->文件夹内容清空后文件夹自动删除";
            InputStream file = new ByteArrayInputStream(txt.getBytes());
            minioclient.putObject(PutObjectArgs.builder().bucket(bucket).object("注意事项").stream(file, file.available(), -1).build());
        }
    }
